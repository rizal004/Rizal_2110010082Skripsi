...
<section class="content-header">     
    <h1 style="text-align:center;">Data Kritik & Saran Wisata</h1>     
    <ol class="breadcrumb">         
        <li>             
            <a href="index.php">                 
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        ...
                        <select name="kategori" id="kategori" class="form-control" onchange="redirectToCategory(this.value)">                         
                            ...
                            <option value="wisata" selected>Kritik dan Saran Wisata</option>                         
            ...
            <div class="table-responsive">                 
                <table id="example1" class="table table-bordered table-striped">                     
                    ...
                    <tbody>                         
                        ...
                        <tr>                             
                            <td><?= $no++; ?></td>                             
                            <td><?= htmlspecialchars($data['kategori'] ?? 'Tidak Diketahui'); ?></td>                             
                            <td><?= htmlspecialchars($data['nama_wisata'] ?? 'Wisata Tidak Ditemukan'); ?></td>                             
                            ...
                            <td>                                 
                                <a href="?page=MyApp/edit_kritik_wisata&id=<?= htmlspecialchars($data['id_kritik_saran']); ?>"                                    
                                   class="btn btn-success btn-sm" title="Ubah">                                     
                                    <i class="glyphicon glyphicon-edit"></i>                                 
                                </a>                                 
                                <a href="?page=MyApp/del_kritik_wisata&id=<?= htmlspecialchars($data['id_kritik_saran']); ?>"                                    
...
$(document).ready(function() {
    $('#example1').DataTable({
        ...
        "order": [[ 6, "desc" ]], // Urutkan berdasarkan tanggal terbaru (kolom tanggal sekarang di index 6)
        "pageLength": 25,
        "columnDefs": [
            { "orderable": false, "targets": [7] }, // Kolom Kelola tidak bisa diurutkan (sekarang index 7)
            { "width": "5%", "targets": [0] }, // No
            { "width": "10%", "targets": [1] }, // Kategori
            { "width": "18%", "targets": [2] }, // Nama Wisata
            { "width": "12%", "targets": [3] }, // Nama Pengguna
            { "width": "10%", "targets": [4] }, // Rating
            { "width": "25%", "targets": [5] }, // Komentar
            { "width": "12%", "targets": [6] }, // Tanggal
            { "width": "8%", "targets": [7] }   // Kelola
        ]
    });
});
