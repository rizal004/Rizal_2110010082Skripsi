...
<style>
body {
    background: linear-gradient(120deg, #6c5ce7 0%, #a29bfe 100%) !important;
...
.content-header h1 { color: #fff !important; text-shadow:1px 2px 8px rgba(108,92,231,0.3); }
.breadcrumb {
...
.box.box-primary, .box.box-info {
    ...
    border-radius: 12px;
    box-shadow: 0 4px 20px 0 rgba(108,92,231,0.06);
...
.btn-info { background: linear-gradient(90deg, #6c5ce7 0%, #a29bfe 100%); border: none; color: #fff;}
.btn-primary { background: linear-gradient(90deg, #6c5ce7 0%, #a29bfe 100%); border: none; color: #fff;}
.btn-success { background: linear-gradient(90deg, #6c5ce7 0%, #a29bfe 100%); border: none; color: #fff;}
.btn-info:hover, .btn-primary:hover, .btn-success:hover { filter: brightness(1.12); }
.table-responsive { border-radius: 16px; box-shadow: 0 4px 18px rgba(108,92,231,0.09); background: #fff;}
.table-bordered>thead>tr { background: linear-gradient(90deg, #6c5ce7 10%, #a29bfe 100%); color: #fff; font-size: 15px;}
.table-bordered>tbody>tr:nth-child(even) { background: #f8f7ff;}
...
.status-cancelled { background: #fd79a8; color: #e84393; }
</style>

<section class="content-header">
    <h1 style="text-align:center; color:#fff !important;">Laporan Data Pemesanan Tiket</h1>
    <ol class="breadcrumb">
        <li>
        ...
        <li>
            <a href="?page=MyApp/data_transaksi" style="color:black;">Transaksi Wisata</a>
        </li>
...
<section class="content">
    <div class="box box-primary">
        ...
        <form method="GET" action="">
            <input type="hidden" name="page" value="MyApp/laporan_tiket">
            <div class="box-body">
                <div class="row">
                    <div class="col-md-3">
                        <div class="form-group">
                            ...
                            <select name="tahun" class="form-control">
                                ...
                                <?php
                                $tahun_query = $koneksi->query("SELECT DISTINCT YEAR(tanggal) as tahun FROM tb_transaksi ORDER BY tahun DESC");
    ...
    <div class="box box-info">
        <div class="box-header with-border" style="background:linear-gradient(90deg, #6c5ce7 0%, #a29bfe 100%);color:#fff;">
        ...
        <div class="box-body">
            <div id="printArea">
                <div class="text-center" style="margin-bottom: 20px;">
                    <h2 style="color:#222;"><b>LAPORAN DATA PEMESANAN TIKET</b></h2>
                    <h4 style="color:#6c5ce7;">Si Wisata</h4>
                    <?php
                    $bulan_array = [
                        1 => 'Januari', 2 => 'Februari', 3 => 'Maret', 4 => 'April',
                        5 => 'Mei', 6 => 'Juni', 7 => 'Juli', 8 => 'Agustus',
                    ...
                    if(isset($_GET['cetak_semua'])) {
                        echo "<p><b>Semua Data Transaksi Wisata</b></p>";
                    } else {
                        $filter_text = [];
                    ...
                    <table class="table table-bordered" style="font-size: 12px;">
                        ...
                        <tbody>
                            ...
                            if(isset($_GET['filter'])) {
                                if(!empty($_GET['tahun'])) {
                                    $where_conditions[] = "YEAR(t.tanggal) = '".$_GET['tahun']."'";
                                }
                                if(!empty($_GET['bulan'])) {
                                    $where_conditions[] = "MONTH(t.tanggal) = '".$_GET['bulan']."'";
                                }
                                if(!empty($_GET['status'])) {
                                    $where_conditions[] = "t.status = '".$koneksi->real_escape_string($_GET['status'])."'";
                                }
                            }
                            $where_clause = '';
                            ...
                            $sql = $koneksi->query("SELECT t.*, w.nama_wisata, p.nama_pengguna AS nama_pemesan
                                                   FROM tb_transaksi t 
                            ...
                            <tr>
                                ...
                                <td style="text-align: center;"><?= $data['jumlah_tiket']; ?> tiket</td>
                                ...
                                <td>
                                    <span class="status-badge status-<?= $data['status']; ?>">
                                        ...
                                        switch($data['status']) {
                                            ...
                                            default: echo ucfirst($data['status']);
                                        }
                                        ?>
                                    </span>
                                </td>
                        ...
                        <tfoot>
                            <tr style="background: #f8f7ff; font-weight: bold;">
                                <td colspan="5" style="text-align: right;">Total Tiket Terjual:</td>
                                <td style="text-align: center;"><?= number_format($total_tiket, 0, ',', '.'); ?> tiket</td>
                ...
                <?php if($total_pendapatan > 0): ?>
                <div style="margin-top: 20px; padding: 15px; background: #f8f7ff; border-radius: 8px;">
                    <h4 style="color: #6c5ce7; margin: 0;">
                    ...
                    <div class="row" style="margin-top: 10px;">
                        <div class="col-md-6">
                            <p style="margin: 5px 0;">
                                Total Tiket Terjual: 
                                <strong style="color: #6c5ce7;"><?= number_format($total_tiket, 0, ',', '.'); ?> tiket</strong>
                            </p>
                        </div>
                        <div class="col-md-6">
                            <p style="margin: 5px 0;">
                                Total Pendapatan: 
                                <strong style="color: #6c5ce7;">Rp <?= number_format($total_pendapatan, 0, ',', '.'); ?></strong>
                            </p>
                        </div>
                    </div>
                </div>
...
<style media="print">
    ...
    .row { display: block; }
    .col-md-6 { display: inline-block; width: 48%; }
</style>